...
AFAC2026 赛道四的完整参赛项目：面向保险条款、监管法规、金融合同、上市公司年报、行业研报五类金融长文本，构建 **"检索规划 → BM25 召回 → 证据压缩 → 综合作答"** 四步 Agent 流水线，在不修改基座模型的前提下完成高准确率、低 Token 消耗的问答。

代码与文档均为原创，仓库结构参考了同类参赛项目的通用组织方式。数据版权归赛题方所有，官方文件（B 榜题目、提交模板）随仓库提供，原始数据集需自行从赛题页面下载。

## 特性

- **四步 Agent 流水线**：检索规划、BM25 召回、LLM 证据压缩、综合作答，各步骤独立模块、可单独调试
...
AFAC2026/
├── agent/                    # 四步 Agent 流水线
│   ├── qa_agent.py           # 编排器：串联四步、Token 记账
│   ├── analyze_step.py       # Step1 检索规划
│   ├── retrieve_step.py      # Step2 BM25 召回
│   ├── compress_step.py      # Step3 证据压缩
│   ├── answer_step.py        # Step4 综合作答（含计算题代码执行）
...
flowchart TD
    ...
    Q[题目] --> C[Step1 检索规划<br/>LLM 拆解选项/计算点]
    C --> D[Step2 BM25 召回<br/>目标文档内 Top-K]
    D --> E[Step3 证据压缩<br/>LLM 只留关键证据]
    E --> F[Step4 综合作答<br/>计算题先代码执行]
    B -. 离线召回 .-> D
...
| [05-Agent流水线](docs/05-Agent流水线.md) | 四步流程细节、计算题代码执行 |
| [06-提交与评测](docs/06-提交与评测.md) | 官方格式、Token 估算、验证工作流 |
| [07-复现与FAQ](docs/07-复现与FAQ.md) | 从零复现、常见坑 |

## 结果与复盘

- A 榜 100 题完整跑通（单题平均约 55 秒，5 线程并发），逐题证据与日志保存在 `output/`（未随仓库发布）。
- B 榜保险领域提交前做了逐题对照条款原文的验证，验证方法见 [06-提交与评测](docs/06-提交与评测.md)（验证报告含答案，仅保留在本地 `output/archive/`）。
- 最终提交文件与验证报告（含答案）属于竞赛答案数据，一律不进入公开仓库。
